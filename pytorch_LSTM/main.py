...
date = start
xlist = [(date + datetime.timedelta(days=i)).strftime("%Y%m%d") for i in range(0, len(df))]
...
plt.legend()
plt.xticks(np.arange(0, len(df), 800))
plt.show()
plt.clf()
...
for j in range(0, 10):
    ...
    list1 = np.array(list1, dtype="object").reshape(-1, 1)
    list1 = list1/100000
    list2 = np.array(list2).reshape(-1, 1)
    list2 = list2 / 100000
    plt.figure(figsize=(10,6)) #plotting
    if type:
        plt.plot(xlist, list1, label='profit') #actual plot
    else:
        plt.plot(xlist, list2, label='profit')  # actual plot
    plt.ylabel('profit (%)')
    plt.title('Time-Series Profit')
    plt.xticks(np.arange(0, 100, 20))
    plt.savefig(f'./{str(date - datetime.timedelta(days=100))} ~ {str(date)}, profit{profit}, type{type}.png')
    plt.clf()