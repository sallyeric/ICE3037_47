...
for name in names:
    ...
    for i in range(len(t)):
        if t[i] > 202011000000:
            start = i
            break
    t = chart.iloc[start:, 0].values
    o = chart.iloc[start:, 1].values
    h = chart.iloc[start:, 2].values
    l = chart.iloc[start:, 3].values
    c = chart.iloc[start:, 4].values
    v = chart.iloc[start:, 5].values
    ...
    chartfile = './macdData/'+name+'MACD.csv'
    chartData = {'일자': t,
                 'MACD':result['MACD'],
