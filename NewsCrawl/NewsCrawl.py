...
def crawler(company, companyCode, startDate, endDate, maxpage):
    date_index = pd.date_range(start='20200101', end='20201231')
    ...
    for date in tqdm(date_list, desc="Progressing..."):
        ...
        while page <= int(maxpage):
            url = 'https://finance.naver.com/news/news_search.nhn?q=' + str(
                companyCode) + '&x=0&y=0&sm=all.basic&pd=4&stDateStart=' + str(date) + '&stDateEnd=' + str(
                date) + '&page=' + str(page)
            ...
            result = {"회사명": company_result, "날짜": date_result, "언론사": source_result, "기사제목": title_result,
                      "링크": link_result, "본문": content_result}
